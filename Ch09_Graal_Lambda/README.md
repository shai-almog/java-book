...
```bash
Ch09_Graal_Lambda$ sam build
Ch09_Graal_Lambda$ sam deploy --guided
```
...
```bash
Ch09_Graal_Lambda$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
Ch09_Graal_Lambda$ sam local start-api
Ch09_Graal_Lambda$ curl http://localhost:3000/
```
...
```bash
Ch09_Graal_Lambda$ sam logs -n HelloWorldFunction --stack-name <Name-of-your-deployed-stack> --tail
```
...
```bash
Ch09_Graal_Lambda$ cd HelloWorldFunction
